export function getDriveAuthorizeUrl(state: string, redirectUri: string, clientId: string): string {
...
export async function exchangeDriveCode(
  ...
  if (!res.ok) throw new Error(`drive_code_exchange_${res.status}: ${await res.text()}`);
  return res.json();
}
...
export async function refreshDriveToken(
  ...
  const res = await fetch("https://oauth2.googleapis.com/token", { method: "POST", body });
  if (!res.ok) throw new Error(`drive_refresh_${res.status}: ${await res.text()}`);
  return res.json();
}
